...
def test_validate_assessment():
    ...
    try:
        response = requests.post(
            "http://localhost:8003/api/validate-assessment",
...
def test_streaming_report():
    ...
    try:
        response = requests.post(
            "http://localhost:8003/api/generate-report/stream",
            json=test_assessment_data,
            headers={"Content-Type": "application/json"},
            stream=True
        )
...
if __name__ == "__main__":
    ...
    print("🔗 Make sure backend server is running on http://localhost:8003")
