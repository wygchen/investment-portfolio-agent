...
For more specific questions about your portfolio strategy, allocation decisions, or individual holdings, I'm here to provide detailed explanations tailored to your investment objectives."""
    ...
    def generate_markdown_report(self, report_data: Dict[str, Any]) -> str:
        """
        Generate a markdown version of the portfolio report
        ...
        """
        try:
            markdown_content = f"""# {report_data.get('report_title', 'Investment Portfolio Analysis')}

**Generated:** {report_data.get('generated_date', datetime.now().strftime('%B %d, %Y'))}  
**Client ID:** {report_data.get('client_id', 'Unknown')}

---

## Executive Summary

{report_data.get('executive_summary', 'Executive summary not available')}

---

## Portfolio Allocation Strategy

{report_data.get('allocation_rationale', 'Allocation rationale not available')}

---

## Investment Selection Rationale

{report_data.get('selection_rationale', 'Selection rationale not available')}

---

## Risk Analysis & Commentary

{report_data.get('risk_commentary', 'Risk commentary not available')}
...
"""
            ...
            if key_recommendations:
                ...
                markdown_content += "No specific recommendations available.\n"
            ...
            if next_steps:
                ...
                markdown_content += "No specific next steps available.\n"
            
            # Add portfolio allocation if available
            if 'portfolio_allocation' in report_data:
                markdown_content += "\n---\n\n## Portfolio Allocation\n\n"
                allocation = report_data['portfolio_allocation']
                for asset_class, percentage in allocation.items():
                    markdown_content += f"- **{asset_class}**: {percentage}%\n"
            
            # Add individual holdings if available
            if 'individual_holdings' in report_data:
                markdown_content += "\n---\n\n## Individual Holdings\n\n"
                markdown_content += "| Security | Symbol | Allocation % | Value ($) |\n"
                markdown_content += "|----------|--------|--------------|----------|\n"
                
                for holding in report_data['individual_holdings']:
                    markdown_content += f"| {holding.get('name', 'N/A')} | {holding.get('symbol', 'N/A')} | {holding.get('allocation_percent', 0):.1f}% | ${holding.get('value', 0):,.0f} |\n"
            
            # Add metadata
            metadata = report_data.get('report_metadata', {})
            if metadata:
                markdown_content += f"\n---\n\n## Report Metadata\n\n"
                markdown_content += f"- **Generated by**: {metadata.get('generated_by', 'Unknown')}\n"
                markdown_content += f"- **Report Type**: {metadata.get('report_type', 'Unknown')}\n"
                markdown_content += f"- **Generated at**: {metadata.get('timestamp', 'Unknown')}\n"
