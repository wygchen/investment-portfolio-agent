  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <Label htmlFor="specific-assets" className="text-base font-semibold">
          Specific Assets & Stock Preferences
        </Label>
        <Textarea
          id="specific-assets"
          placeholder="Enter specific stocks, ETFs, or assets you'd like to include (e.g., AAPL, MSFT, VTI, Tesla, Bitcoin, etc.)"
          value={profile.values.specificAssets.join(", ")}
          onChange={(e) => {
            ...
            updateProfile({
              values: { ...profile.values, specificAssets: assets },
            })
          }}
          className="min-h-[100px] resize-none"
        />
        <p className="text-sm text-muted-foreground">
          List specific stock tickers, ETFs, or assets you want considered for your portfolio. Separate multiple entries with commas.
        </p>
      </div>
      ...
      {/* Summary */}
      {(profile.values.avoidIndustries.length > 0 || profile.values.preferIndustries.length > 0 || profile.values.specificAssets.length > 0 || profile.esgPrioritization || profile.marketSelection.length > 0) && (
        <Card className="p-6 bg-muted/50">
          ...
          <div className="space-y-4">
            ...
            {profile.values.specificAssets.length > 0 && (
              <div>
                <p className="text-sm text-muted-foreground mb-2">Specific Assets:</p>
                <div className="flex flex-wrap gap-2">
                  {profile.values.specificAssets.map((asset) => (
                    <Badge key={asset} className="gap-1 bg-orange-500 text-white">
