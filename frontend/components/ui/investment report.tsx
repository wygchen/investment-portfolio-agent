...
export function InvestmentReportComponent() {
  ...
  const generateReport = async () => {
    ...
    try {
      // Try to get user assessment data from localStorage first
      const storedAssessment = localStorage.getItem('portfolioai_assessment')
      let usePersonalizedEndpoint = false
      let assessmentData = null
      
      if (storedAssessment) {
        try {
          assessmentData = JSON.parse(storedAssessment)
          usePersonalizedEndpoint = true
        } catch (e) {
          console.log("Failed to parse stored assessment data, using default report")
        }
      }
      ...
      // Add assessment data if we have it
      if (usePersonalizedEndpoint && assessmentData) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/download-report/${report.pdf_filename}`)
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `investment_report_${new Date().toISOString().split('T')[0]}.pdf`
